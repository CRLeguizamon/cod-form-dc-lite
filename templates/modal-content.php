 ...
 * @author      Cristian Leguizamón - crleguizamon
 * @version     1.0
 * 
...
// ==============================================
$modal_title = get_option('cod_form_modal_title', esc_html__('Pago contra entrega', 'modal-cod-form'));
$modal_title_size = get_option('cod_form_modal_title_size', esc_html__('28', 'modal-cod-form'));
...
$shipping_table = $ignore_wc_shipping == 'no' ? wc_price(0) : esc_html__('Gratis', 'modal-cod-form');
...
if (!empty($terms_page_id)) {
    ...
    $terms_link_html = '<a href="' . esc_url($terms_link) . '" target="_blank">' . get_option('cod_form_terms_checkbox_link_text', esc_html__('términos y condiciones', 'modal-cod-form')) . '</a>';
} else {
    $terms_link_html = '';
}
...
<span class="cod-modal-close button_modal_close">
    <img width="15" src="<?php echo esc_url(MODALCODF_DC_ASSETS_URL . 'img/cod_x_black.png'); ?>" alt="<?php esc_attr_e('Cerrar modal', 'modal-cod-form'); ?>">
</span>
<?php if (empty($cart_items)) : ?>
    <div class="not_cart_items">
        ...
        <h4><?php echo esc_html__('No hay productos en tu carrito de compra.', 'modal-cod-form'); ?></h4>
    </div>
<?php else : ?>
<?php 
    if (!empty($image_banner_top)) {
        echo '<img src="' . esc_url($image_banner_top) . '" alt="' . esc_attr__('Banner superior', 'modal-cod-form') . '" style="max-width:100%;">';
    }
?>
            ...
            <tr class="cod-cart woocommerce-cart-form__cart-item cod_cart_item" data="<?php echo esc_attr($product->get_id()); ?>">
                ...
                <td class="cod-product-remove">
                    <button type="button" class="remove-product" data-cart_item_key="<?php echo esc_attr($cart_item_key); ?>">
                        <img width="15" src="<?php echo esc_url(MODALCODF_DC_ASSETS_URL . 'img/cod_x_red.png'); ?>" alt="<?php esc_attr_e('Eliminar producto', 'modal-cod-form'); ?>">
                    </button>
                </td>
            </tr>
        <?php endforeach; ?>
...
<form id="checkout_form" action="#" method="post" data-currency="<?php echo esc_attr($currency_symbol); ?>">
    ...
    <?php if ($country_selection === 'enable_select') : ?>
        <div class="<?php echo esc_attr($form_group_class); ?>">
            <label for="country" ><?php echo esc_html__('País', 'modal-cod-form'); ?><div class="cod_required_input">*</div></label>
            <select style="background-image:url(<?php echo esc_url(MODALCODF_DC_ASSETS_URL.'img/location.svg'); ?>);" class="form-control" id="country" name="country" required>
                <option value=""><?php echo esc_html__('Seleccionar país', 'modal-cod-form'); ?></option>
    ...
    <div class="<?php echo esc_attr($form_group_class); ?>">
        ...
        <input style="background-image:url(<?php echo esc_url(MODALCODF_DC_ASSETS_URL.'img/user.svg'); ?>);" type="text" class="form-control" id="first_name" name="first_name" placeholder="<?php echo esc_attr($first_name_placeholder); ?>" required>
    </div>
    <div class="<?php echo esc_attr($form_group_class); ?>">
        ...
        <input style="background-image:url(<?php echo esc_url(MODALCODF_DC_ASSETS_URL.'img/user.svg'); ?>);" type="text" class="form-control" id="last_name" name="last_name" placeholder="<?php echo esc_attr($last_name_placeholder); ?>" required>
    </div>
    <div class="<?php echo esc_attr($form_group_class); ?>">
        ...
        <input style="background-image:url(<?php echo esc_url(MODALCODF_DC_ASSETS_URL.'img/phone.svg'); ?>);" type="tel" class="form-control" id="phone" name="phone" placeholder="<?php echo esc_attr($phone_placeholder); ?>" required>
    </div>
    <div class="<?php echo esc_attr($form_group_class); ?>">
        ...
        <select style="background-image:url(<?php echo esc_url(MODALCODF_DC_ASSETS_URL.'img/location.svg'); ?>);" class="form-control <?php echo esc_attr($select_state); ?>" id="state" name="state" required>
    ...
    <div class="<?php echo esc_attr($form_group_class); ?>">
        ...
        <input style="background-image:url(<?php echo esc_url(MODALCODF_DC_ASSETS_URL.'img/location.svg'); ?>);" type="text" class="form-control" id="city" name="city" placeholder="<?php echo esc_attr($city_placeholder); ?>" required>
    </div>
    ...
    <div class="<?php echo esc_attr($form_group_class); ?>">
        ...
        <input style="background-image:url(<?php echo esc_url(MODALCODF_DC_ASSETS_URL.'img/location.svg'); ?>);" type="text" class="form-control" id="address" name="address" placeholder="<?php echo esc_attr($address_placeholder); ?>" required>
    </div>
    <?php if (!is_field_disabled('address_2', $disabled_fields)): ?>
        <div class="<?php echo esc_attr($form_group_class); ?>">
            ...
            <input style="background-image:url(<?php echo esc_url(MODALCODF_DC_ASSETS_URL.'img/location.svg'); ?>);" type="text" class="form-control" id="address_2" name="address_2" placeholder="<?php echo esc_attr($address_2_placeholder); ?>" required>
        </div>
    <?php endif; ?>
    ...
    <?php if (!is_field_disabled('email', $disabled_fields)): ?>
        <div class="<?php echo esc_attr($form_group_class); ?>">
            ...
            <input style="background-image:url(<?php echo esc_url(MODALCODF_DC_ASSETS_URL.'img/email.svg'); ?>);" type="email" class="form-control" id="email" name="email" <?php if($is_email_mandatory == 'no'): echo 'placeholder="' . esc_attr__('Opcional', 'modal-cod-form') . '"'; else: echo 'placeholder="' . esc_attr($email_placeholder) . '" required';endif; ?> >
        </div>
    <?php endif; ?>
    ...
    <?php if (!is_field_disabled('order_comments', $disabled_fields)) : ?>
        <div class="<?php echo esc_attr($form_group_class); ?> cod_notes">
            ...
            <input style="background-image:url(<?php echo esc_url(MODALCODF_DC_ASSETS_URL.'img/msj.svg'); ?>);" type="text" class="form-control" id="order_comments" name="order_comments" placeholder="<?php echo esc_attr($order_notes_placeholder); ?>" >
        </div>
    <?php endif; ?>
    ...
    <?php if (!is_field_disabled('terms', $disabled_fields)) : ?>
        <div class="form-group cod_terms">
            <label for="terms_checkbox">
                ...
                <?php echo esc_html(get_option('cod_form_terms_checkbox_text', esc_html__('I agree to the terms and conditions', 'modal-cod-form'))) . ' ' . wp_kses_post($terms_link_html); ?>
            </label>
        </div>
    <?php endif; ?>
    ...
    <?php if (!empty($image_banner_bottom)) {
        echo '<img src="' . esc_url($image_banner_bottom) . '" alt="' . esc_attr__('Footer', 'modal-cod-form') . '" style="max-width:100%;">';
    } ?>
